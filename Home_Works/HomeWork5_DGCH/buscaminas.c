...
char hash[CONTL][CONTL]; 	//Muestra los arrays
int nums[CONTL][CONTL]; 	//Muestra columnas y filas
	...
	while(wait) { //if the imput was invalid it reapeats
		...
		while((c = getchar()) != EOF && c != '\n' && wait) { //takes imput
			if(c == '0' && save)
				...
				printf("Locacion invalida\n");
				did = 1;
			}	
		}	
		a = pos/10, b = pos%10;
		if(hash[a][b] == '-' && !block)
			did = 1, printf("Espacio Bloqueado\n");
		if(hash[a][b] != '#' && hash[a][b] != '-')
			did = 1, printf("Ya abierto\n");	
		if(!did)
			wait = 0;
	}
	printf("\n");
